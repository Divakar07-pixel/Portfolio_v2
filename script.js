...
if(canvas&&stage){
 ...
 function create(){
  ...
  points=Array.from({length:POINTS},(_,i)=>{const band=i%5,a=Math.random()*Math.PI*2,u=Math.random()*2-1,rr=r*(.68+Math.random()*.34);return{x:Math.sqrt(1-u*u)*Math.cos(a)*rr,y:u*rr,z:Math.sqrt(1-u*u)*Math.sin(a)*rr,size:.45+Math.random()*.9,phase:Math.random()*6.28,speed:.00006+Math.random()*.00007,band};});
 }
 ...
 function draw(t){
  ctx.clearRect(0,0,w,h);tx+=(mx-tx)*.035;ty+=(my-ty)*.035;const cx=mx*w,cy=my*h,rad=Math.min(w,h)*.27;const active=Math.max(0,1-Math.hypot(cx-w*.5,cy-h*.49)/rad);interaction+=(active-interaction)*.055;const ry=t*.00009+tx*.18,rx=Math.sin(t*.00013)*.08+ty*.08;
  const pts=points.map(p=>{const drift=Math.sin(t*p.speed+p.phase)*2.8,q=project({x:p.x+drift*Math.cos(p.phase),y:p.y+drift*.45,z:p.z+drift*Math.sin(p.phase)},ry,rx),dx=q.x-cx,dy=q.y-cy,d=Math.hypot(dx,dy);if(d<rad&&d>1){const pull=(1-d/rad)*interaction*10;q.x+=dx/d*pull;q.y+=dy/d*pull;}q.near=Math.max(0,1-d/(rad*1.35));return q;});
  const maxD=Math.min(w,h)*(.13+interaction*.035),links=[];for(let i=0;i<pts.length;i++)for(let j=i+1;j<pts.length;j++){const a=pts[i],b=pts[j],d=Math.hypot(a.x-b.x,a.y-b.y);if(d<maxD&&Math.abs(a.z-b.z)<105)links.push({a,b,d});}links.sort((a,b)=>a.d-b.d);ctx.lineWidth=.45;
  ...
  pts.sort((a,b)=>a.z-b.z).forEach(p=>{const depth=Math.max(0,Math.min(1,(p.z+240)/480)),near=p.near,sz=p.size*(1+near*interaction*.7);ctx.fillStyle=`rgba(255,255,255,${Math.min(.92,.15+depth*.62+near*interaction*.2)})`;ctx.beginPath();ctx.arc(p.x,p.y,Math.max(.42,sz),0,Math.PI*2);ctx.fill();});requestAnimationFrame(draw);
 }
...
if(loader&&stage&&!reduced){
 document.body.classList.add('portal-lock');let entered=false,current=0,target=0,raf=false;const clamp=(v,a,b)=>Math.min(b,Math.max(a,v)),ease=v=>v<.5?2*v*v:1-Math.pow(-2*v+2,2)/2;
